...
router.get('/folders', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: error instanceof Error ? error.message : 'Failed to list folders' });
  }
});
...
router.get('/images/:folder?', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: error instanceof Error ? error.message : 'Failed to list images' });
  }
});
...
router.post('/generate', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: error instanceof Error ? error.message : 'Failed to generate NFTs' });
  }
});
...
router.post('/import', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: error instanceof Error ? error.message : 'Failed to import NFTs' });
  }
});
