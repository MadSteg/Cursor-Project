 ...
 * 
 * This component provides an interface for users to manage their Taco 
...
// Mock function for local key storage - in a real app, this would be more secure
const saveKeyToLocalStorage = (keyName: string, privateKey: string) => {
  try {
    const keys = JSON.parse(localStorage.getItem('tacoPrivateKeys') || '{}');
    keys[keyName] = privateKey;
    localStorage.setItem('tacoPrivateKeys', JSON.stringify(keys));
    return true;
  } catch (error) {
...
export default function TacoKeyManager() {
  ...
  const { data: keys = [], isLoading } = useQuery({
    queryKey: ['/api/taco/keys'],
    queryFn: async () => {
      // In a real app, we would get the actual user ID
      const userId = 1;
      return tacoThresholdCrypto.getKeys(userId);
    },
  });
    ...
    <Card className="w-full">
      <CardHeader>
        <div className="flex justify-between items-center">
          <div>
            <CardTitle>Taco Threshold Encryption Keys</CardTitle>
            <CardDescription>
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  This key will allow you to securely share receipts with others using Taco threshold encryption
                </DialogDescription>
              </DialogHeader>
              <Form {...form}>
